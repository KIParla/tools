...
|`SWITCHES`
|Moves intonation markers (`. , ?`) before prosodic/interruption symbols (`: - ~`) when they appear in the wrong order; moves NVB tags and shortpause `(.)` outside guess/pace/volume spans (both are exempt at overlap-span `[…]` edges, unconditionally)
|yes
|===
...
== NVB tags, shortpause, and bracket spans

Non-verbal behaviour tags (`((laugh))`, `((ride))`, `((toss))`, …) and shortpause `(.)` must appear _outside_
guess/pace/volume bracket spans (`(…)`, `<…>`, `°…°`), not inside them. The `SWITCHES` warning rule (`switch_NVB`)
enforces this automatically by relocating any NVB tag or `(.)` found immediately after an opening bracket or
immediately before a closing one.

**Overlap brackets `[…]` are the one exception, and it is unconditional:** an NVB tag or `(.)` immediately after `[`
or immediately before `]` is always left in place, regardless of any config. Both are transcriptionally valid content
of an overlap span, not something to relocate out of it — relocating would leave a bracket pair with no content of
its own, which a later rule then deletes as an empty span, silently destroying the overlap annotation.

This is purely a position rule and has nothing to do with `overlaps.nvb_participates_in_overlaps`. That config
instead governs a separate, TU-level decision: whether a TU that consists *entirely* of NVB and/or shortpause tokens
is treated as participating in the time-based overlap graph at all — see `Transcript.check_overlaps` (`data.py`,
`PIPELINE.md` step 6, sections 6a/6d/6e).

[source]
----
[(.)]         ← valid, no change   (pause always exempt at the edge)
[((laugh))]   ← valid, no change   (NVB always exempt at the edge)
[((laugh)) ciao]  ← valid, no change
((.))         ← becomes (.) ()    (guess span: unrelated to overlaps, always relocated)
<((laugh))>   ← becomes ((laugh)) <>   (pace span: same)
----
...
* the unconditional overlap-bracket-edge exemption for NVB and shortpause `(.)` in `switch_NVB`
