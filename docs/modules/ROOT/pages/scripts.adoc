...
Builds the xref:validation-report.adoc[Validation report] page: combines `check_participants.py`'s metadata-consistency results with per-conversation pipeline warnings/errors (from each module's `tmp/process/json/summary.json`, produced by `cli.py process`) into one AsciiDoc table per module, sorted so files needing attention come first.
...
Run it again (and commit the regenerated page) after reprocessing a module, to keep the report current.
